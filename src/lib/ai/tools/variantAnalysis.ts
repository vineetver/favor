...
export function getVariantAnalysis() {
  return tool({
    description:
      'Analyze specific known variants by VCF ID (e.g., "19-44908822-C-T") or rsID (e.g., "rs429358"). Gets detailed information about individual variants including population frequencies, functional scores, regulatory elements, and GWAS associations. Do NOT use this for listing variants within genes or regions - use geneAnalysis or regionAnalysis tools instead.',
    inputSchema: z.object({
      variants: z
        .array(z.string())
        .optional()
        .describe(
          'Array of variant VCF strings (e.g., ["19-44908822-C-T", "1-100-A-G"]) or rsIDs (e.g., ["rs429358", "rs7412"])',
        ),
      variant: z
        .string()
        .optional()
        .describe(
          "Single variant in VCF format (chr:pos:ref:alt) or rsID (e.g., rs7412)",
        ),
      ...
      dataSources: z
        ...
        .optional()
        .describe(
          "Data sources to include (default: basic + functional_scores + regulatory_elements)",
        ),
      ...
      aggregationLevel: z
        ...
        .optional()
        .describe(
          "Level of data aggregation - summary: key metrics only, detailed: expanded analysis, full: all available data",
        ),
      ...
      includeGenderSpecific: z
        .boolean()
        .optional()
        .describe("Include gender-specific frequency data"),

      // Functional analysis
      functionalScoreTypes: z
        .array(z.enum(["scent", "cv2f", "pgboost", "spliceai", "all"]))
        .nullable()
        .optional()
        .describe("Types of functional scores to include"),
      minFunctionalScore: z
        .number()
        .nullable()
        .optional()
        .describe("Minimum threshold for functional prediction scores"),
      tissueFilter: z
        .array(z.string())
        .nullable()
        .optional()
        .describe("Filter SCENT and regulatory data by specific tissues"),
      ...
      maxPValue: z
        ...
        .nullable()
        .optional()
        .describe("Maximum p-value for GWAS associations (e.g., 5e-8)"),
      gwasTraits: z
        .array(z.string())
        .nullable()
        .optional()
        .describe("Filter GWAS data by specific traits or diseases"),
      minSampleSize: z
        .number()
        .nullable()
        .optional()
        .describe("Minimum study sample size for GWAS associations"),

      // Regulatory analysis
      regulatoryDistance: z
        .number()
        .nullable()
        .optional()
        .describe(
          "Distance around variant to search for regulatory elements (default: 0)",
        ),
      regulatoryTypes: z
        .array(z.string())
        .nullable()
        .optional()
        .describe(
          'Filter regulatory elements by type (e.g., ["pELS", "dELS", "PLS"])',
        ),
      ...
      maxResultsPerSource: z
        ...
        .optional()
        .describe(
          "Maximum results to return per data source (useful for large datasets)",
        ),
      prioritizeByRelevance: z
        .boolean()
        .optional()
        .describe("Prioritize results by clinical/functional relevance"),
      includeVisualizationData: z
        .boolean()
        .optional()
        .describe("Include data formatted for visualization components"),

      // Cross-variant analysis (for multiple variants)
      performCrossAnalysis: z
        .boolean()
        .optional()
        .describe(
          "When analyzing multiple variants, perform comparative analysis",
        ),
      identifySharedFeatures: z
        .boolean()
        .optional()
        .describe(
          "Identify shared regulatory elements, pathways, or traits across variants",
        ),
    }),
    execute: async ({
      variants,
      ...
      const normalizedOptions = {
        ...
        functionalScoreTypes: functionalScoreTypes || [
          "scent",
          "cv2f",
          "pgboost",
        ],
      ...
      const results: any = {
        ...
        analysisType:
          variantList.length > 1 ? "multi_variant" : "single_variant",
        dataSources,
        aggregationLevel,
        data: {},
      };
      ...
      if (
        ...
        (performCrossAnalysis || identifySharedFeatures)
      ) {
      ...
      // Generate summary
      results.summary = generateAnalysisSummary(
        results.data,
        variantList.length,
      );
...
async function analyzeVariant(variantInput: string, options: any) {
  ...
  if (options.dataSources.includes("population_frequencies")) {
    dataFetches.push(
      Promise.all([fetchGnomadExome(variantId), fetchGnomadGenome(variantId)]),
    );
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === "fulfilled" && result.value) {
      if (key === "basic") {
        variantData.basic = aggregateBasicInfo(
          result.value,
...
function aggregatePopulationData(data: any, options: any) {
  ...
  ["exome", "genome"].forEach((dataset) => {
    ...
    if (options.aggregationLevel !== "summary") {
      ...
      if (d.spliceai_ds_max !== undefined)
        functionalScores.spliceai = d.spliceai_ds_max;
      if (d.sift_max !== undefined) functionalScores.sift = d.sift_max;
      if (d.polyphen_max !== undefined)
        functionalScores.polyphen = d.polyphen_max;
...
async function aggregateFunctionalScores(
  variantId: string,
  ...
  if (
    options.functionalScoreTypes.includes("scent") ||
    options.functionalScoreTypes.includes("all")
  ) {
    try {
      ...
      if (scentData) {
        ...
        if (options.aggregationLevel === "summary") {
          ...
          const topTissues = scentData
            .sort(
              (a: any, b: any) => (b.scent_score || 0) - (a.scent_score || 0),
            )
            .slice(0, 5)
            .map((item: any) => ({
              tissue: item.tissue,
          ...
          functionalData.scent = {
            ...
            maxScore: Math.max(
              ...scentData.map((item: any) => item.scent_score || 0),
            ),
          };
        } else {
          functionalData.scent = scentData.slice(
            0,
            options.maxResultsPerSource || 50,
          );
        }
      }
    } catch (error) {
      errors.push(
        `SCENT: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  if (
    rsid &&
    (options.functionalScoreTypes.includes("cv2f") ||
      options.functionalScoreTypes.includes("all"))
  ) {
    try {
      ...
      errors.push(
        `CV2F: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  if (
    rsid &&
    (options.functionalScoreTypes.includes("pgboost") ||
      options.functionalScoreTypes.includes("all"))
  ) {
    try {
      const pgboostData = await fetchPGBoost(rsid);
      if (pgboostData) {
        const pgArray = Array.isArray(pgboostData)
          ? pgboostData
          : [pgboostData];
      ...
      errors.push(
        `PGBoost: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
async function aggregateRegulatoryData(variantId: string, options: any) {
  ...
  try {
    ...
    if (ccreData) {
      ...
      if (options.aggregationLevel === "summary") {
        ...
        regulatoryData.ccre = filteredCCRE.slice(
          0,
          options.maxResultsPerSource || 100,
        );
      }
    }
  } catch (error) {
    errors.push(
      `CCRE: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    if (entexData) {
      if (options.aggregationLevel === "summary") {
        ...
        regulatoryData.entex = {
          ...
          significantImbalances: entexData.filter(
            (item: any) => item.imbalance_significance < 0.05,
          ).length,
        };
      } else {
        regulatoryData.entex = entexData.slice(
          0,
          options.maxResultsPerSource || 50,
        );
      }
    ...
    errors.push(
      `ENTEx: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    if (abcPeaks || abcScores) {
      if (options.aggregationLevel === "summary") {
        const peakTissues =
          abcPeaks?.reduce((acc: any, item: any) => {
            acc[item.tissue] = (acc[item.tissue] || 0) + 1;
            return acc;
          }, {}) || {};

        const scoreTissues =
          abcScores?.reduce((acc: any, item: any) => {
            acc[item.tissue] = (acc[item.tissue] || 0) + 1;
            return acc;
          }, {}) || {};
        ...
        regulatoryData.abc = {
          peaks: {
            ...
            significantPeaks:
              abcPeaks?.filter((item: any) => item.q_value < 0.05).length || 0,
          },
          scores: {
            total: abcScores?.length || 0,
            tissueDistribution: scoreTissues,
            highScores:
              abcScores?.filter((item: any) => item.abc_score > 0.1).length ||
              0,
            averageScore: abcScores?.length
              ? abcScores.reduce(
                  (sum: number, item: any) => sum + item.abc_score,
                  0,
                ) / abcScores.length
              : 0,
          },
    ...
    errors.push(
      `ABC: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
async function aggregateGWASData(variantId: string, options: any) {
  try {
    ...
    if (options.maxPValue !== undefined) {
      gwasData = gwasData.filter(
        (item: any) => parseFloat(item.gwas_p_value) <= options.maxPValue,
      );
    }
    ...
    if (options.minSampleSize !== undefined) {
      gwasData = gwasData.filter((item: any) => {
        const sampleSize = parseInt(
          item.gwas_initial_sample_size.replace(/,/g, ""),
        );
    ...
    if (options.aggregationLevel === "summary") {
      ...
      return {
        ...
        significantAssociations: gwasData.filter(
          (item: any) => parseFloat(item.gwas_p_value) < 5e-8,
        ).length,
      };
    }
    ...
    return {
      error: `GWAS: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

async function performCrossVariantAnalysis(
  variantDataList: any[],
  options: any,
) {
  ...
  if (options.identifySharedFeatures) {
    ...
    const regulatoryTypeCounts = allRegulatoryTypes.reduce(
      (acc: any, type: string) => {
    ...
    crossAnalysis.sharedRegulatoryElements = Object.entries(
      regulatoryTypeCounts,
    )
      .filter(([_, count]) => (count as number) > 1)
...
function generateAnalysisSummary(variantData: any, variantCount: number) {
  const successful = Object.values(variantData).filter(
    (v: any) => !v.error,
  ).length;
  ...
  const availability = dataTypes.reduce((acc: any, type: string) => {
    const hasData = Object.values(variantData).filter(
      (v: any) => v[type] && !v.error,
    ).length;
    ...
    if (data.populationFrequencies) {
      ["exome", "genome"].forEach((dataset) => {
        if (data.populationFrequencies[dataset]?.ancestries) {
          const frequencies = chartData.labels.map(
            (ancestry: string) =>
...
function generateFunctionalScoresHeatmap(variantData: any) {
  ...
  Object.entries(variantData).forEach(([variant, data]: [string, any]) => {
    console.log(
      `🔍 Debug - Processing variant ${variant}:`,
      Object.keys(data || {}),
    );
    ...
    if (data.populationFrequencies) {
      ["exome", "genome"].forEach((dataset) => {
        const scores =
          data.populationFrequencies[dataset]?.predictionScores || {};
        console.log(
          `🔍 Debug - ${variant} ${dataset} prediction scores:`,
          scores,
        );
    ...
    if (data.functionalScores) {
      console.log(
        `🔍 Debug - ${variant} functional scores:`,
        Object.keys(data.functionalScores),
      );

      // SCENT scores - get max score across tissues
      if (data.functionalScores.scent) {
        console.log(
          `🔍 Debug - ${variant} SCENT data:`,
          data.functionalScores.scent,
        );
        let maxScentScore = 0;
        ...
        if (data.functionalScores.scent.maxScore) {
          ...
          maxScentScore = Math.max(
            ...data.functionalScores.scent.topTissues.map(
              (t: any) => t.score || 0,
            ),
          );
        } else if (Array.isArray(data.functionalScores.scent)) {
          maxScentScore = Math.max(
            ...data.functionalScores.scent.map((t: any) => t.scent_score || 0),
          );
        }
      ...
      if (data.functionalScores.pgboost) {
        const pgData = Array.isArray(data.functionalScores.pgboost)
          ? data.functionalScores.pgboost[0]
          : data.functionalScores.pgboost;
  ...
  if (heatmapData.length === 0) {
    console.log(
      "🔍 Debug - No heatmap data found, available variant data keys:",
      ...
      if (heatmapData.length > 0) {
        return {
          ...
          metadata: {
            ...
            scoreTypes: Array.from(
              new Set(heatmapData.map((d) => d.scoreType)),
            ),
    ...
    return {
      ...
      suggestion:
        "Try analyzing variants with functional_scores data source enabled, or use variants known to have prediction scores like rs7412 or rs1799853",
      data: [],
      xAxis: "variant",
      yAxis: "scoreType",
      value: "score",
      debug: {
        ...
        availableDataTypes: Object.keys(variantData).map((v) =>
          Object.keys(variantData[v] || {}),
        ),
      },
    };
  }
...
function generateRegulatoryLandscape(variantData: any) {
  ...
  Object.entries(variantData).forEach(([variant, data]: [string, any]) => {
    if (data.regulatoryElements?.ccre?.annotationSummary) {
      Object.entries(data.regulatoryElements.ccre.annotationSummary).forEach(
        ([annotation, count]: [string, any]) => {
          landscapeData.push({
            variant,
            annotation,
            count,
...
function generateCrossVariantComparison(variantData: any) {
  ...
  const metrics = [
    "totalGWASAssociations",
    "functionalScores",
    "regulatoryElements",
  ];

  metrics.forEach((metric) => {
    comparisonData.metrics[metric] = Object.entries(variantData).map(
