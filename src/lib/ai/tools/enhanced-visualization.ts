...
export function createScatterPlot() {
  return tool({
    ...
    execute: async ({ data, title, xLabel, yLabel, width = 600, height = 400 }: {
      data: Array<{x: number; y: number; label?: string; color?: string; size?: number}>;
      title: string;
      xLabel: string;
      yLabel: string;
      width?: number;
...
export function createBarChart() {
  return tool({
    ...
    inputSchema: z.object({
      ...
      xLabel: z.string().describe('X-axis label'),
      yLabel: z.string().describe('Y-axis label'),
      orientation: z.enum(['horizontal', 'vertical']).optional().describe('Bar orientation'),
      width: z.number().optional().describe('Chart width in pixels'),
      height: z.number().optional().describe('Chart height in pixels')
    }),
    execute: async ({ data, title, xLabel, yLabel, orientation = 'vertical', width = 600, height = 400 }: {
...
export function createLineChart() {
  return tool({
    description: 'Create a line chart visualization for time series or sequential data',
    inputSchema: z.object({
      data: z.array(z.object({
    ...
    execute: async ({ data, title, xLabel, yLabel, smooth = false, width = 600, height = 400 }: {
...
export function createHeatmap() {
  return tool({
    ...
    inputSchema: z.object({
      ...
      title: z.string().describe('Chart title'),
      colorScale: z.enum(['viridis', 'plasma', 'blues', 'reds']).optional().describe('Color scale'),
      width: z.number().optional().describe('Chart width in pixels'),
      height: z.number().optional().describe('Chart height in pixels')
    }),
    execute: async ({ data, rowLabels, colLabels, title, colorScale = 'viridis', width = 600, height = 400 }: {
      data: number[][];
      rowLabels?: string[];
      colLabels?: string[];
      title: string;
...
export function createNetworkGraph() {
  return tool({
    description: 'Create a network graph visualization for relationships and interactions',
    inputSchema: z.object({
      ...
      layout: z.enum(['force', 'circular', 'hierarchical']).optional().describe('Layout algorithm'),
      width: z.number().optional().describe('Graph width in pixels'),
      height: z.number().optional().describe('Graph height in pixels')
    }),
    execute: async ({ nodes, edges, title, layout = 'force', width = 800, height = 600 }: {
      nodes: Array<{id: string; label: string; size?: number; color?: string; group?: string}>;
      edges: Array<{source: string; target: string; weight?: number; color?: string; label?: string}>;
      title: string;
...
export function createManhattanPlot() {
  return tool({
    ...
    inputSchema: z.object({
      ...
      significanceThreshold: z.number().optional().describe('P-value significance threshold'),
      width: z.number().optional().describe('Plot width in pixels'),
      height: z.number().optional().describe('Plot height in pixels')
    }),
    execute: async ({ data, title, significanceThreshold = 5e-8, width = 1200, height = 400 }: {
      data: Array<{chromosome: string; position: number; pvalue: number; snp?: string; gene?: string}>;
      title: string;
...
export function createVolcanoPlot() {
  return tool({
    ...
    execute: async ({ data, title, fcThreshold = 1, pThreshold = 0.05, width = 600, height = 500 }: {
      data: Array<{gene: string; logFC: number; pvalue: number; significant?: boolean}>;
      title: string;
      fcThreshold?: number;
      pThreshold?: number;
      width?: number;
