// Comprehensive field descriptions for genomic data fields
export const variantInfo: Record<string, string> = {
  variant_vcf:
...
// Approximate matching function with fuzzy string matching
function approximateMatch(
  ...
  const maxAllowedDistance = Math.max(2, Math.floor(field.length * 0.3)); // Allow up to 30% character differences
...
function normalize(s: string): string {
  return s.toLowerCase()
    .replace(/[\s\-_\.]/g, '') // Remove spaces, hyphens, underscores, periods
    .replace(/[^\w]/g, ''); // Remove any remaining non-word characters
}

// Enhanced field description fetching with fuzzy matching
export async function fetchFieldDescription(field: string): Promise<string> {
  // First try exact match (case insensitive)
  const exactMatch = Object.keys(variantInfo).find(
    key => key.toLowerCase() === field.toLowerCase()
  );
  
  if (exactMatch) {
    return variantInfo[exactMatch];
  }
  ...
  if (closestMatch) {
    return `${variantInfo[closestMatch]} (matched "${closestMatch}" for "${field}")`;
  }

  return `Sorry, I did not find a description for "${field}". Please try a different field name or check the spelling.`;
}

// Alternative interface for integration with existing code
export { variantInfo as fieldDescriptions };