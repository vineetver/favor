...
export class AIErrorHandler {
  ...
  private enhanceError(error: unknown, context?: string, attempt?: number): Error {
    if (error instanceof ChatSDKError) {
      return error;
    }
    ...
    return this.withRetry(async () => {
      try {
        ...
        if (typeof navigator !== 'undefined' && !navigator.onLine) {
          throw new ChatSDKError('offline:chat', 'No internet connection available');
        }
  ...
  handleModelError(error: unknown, modelId: string): Error {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('context length') || error.message.includes('token limit')) {
        return new ChatSDKError('bad_request:chat', 
      ...
      if (error.message.includes('quota') || error.message.includes('billing')) {
        return new ChatSDKError('forbidden:chat', 
