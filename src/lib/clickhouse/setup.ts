...
export function printConnectionInfo(): void {
  ...
  console.log(
    `   CA Cert: ${process.env.CLICKHOUSE_CA_CERT ? "✅ Set" : "❌ Not set"}`,
  );
  console.log(
    `   Client Cert: ${process.env.CLICKHOUSE_CLIENT_CERT ? "✅ Set" : "❌ Not set"}`,
  );
  console.log(
    `   Client Key: ${process.env.CLICKHOUSE_CLIENT_KEY ? "✅ Set" : "❌ Not set"}`,
  );
}
...
export async function diagnoseConnection(): Promise<void> {
  ...
  try {
    ...
    if (pingResult) {
      ...
      console.log(
        `   Result: ${Array.isArray(queryResult) && queryResult.length > 0 ? "✅ Success" : "❌ Failed"}`,
      );
