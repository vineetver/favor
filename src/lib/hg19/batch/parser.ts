...
function parseCSVVariants(lines: string[], sampleId: string): BatchVariantInput[] {
  const variants: BatchVariantInput[] = [];
  ...
  for (const line of dataLines) {
    ...
    if (fields.length >= 4) {
      const variant = createVariant(fields[0], fields[1], fields[2], fields[3], sampleId);
...
function parseTSVVariants(lines: string[], sampleId: string): BatchVariantInput[] {
  const variants: BatchVariantInput[] = [];
  const hasHeader = lines[0]?.toLowerCase().includes('chrom');
  const dataLines = hasHeader ? lines.slice(1) : lines;
...
function parseVCFVariants(lines: string[], sampleId: string): BatchVariantInput[] {
  const variants: BatchVariantInput[] = [];
  ...
  for (const line of lines) {
    ...
    if (fields.length >= 5) {
      ...
      const variant = createVariant(fields[0], fields[1], fields[3], fields[4], sampleId, rsid);
      if (variant) variants.push(variant);
    }
  }
  return variants;
...
function parsePlainTextVariants(lines: string[], sampleId: string): BatchVariantInput[] {
  const variants: BatchVariantInput[] = [];
