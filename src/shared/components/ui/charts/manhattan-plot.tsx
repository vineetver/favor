...
import { PLOTLY_AXIS, PLOTLY_CONFIG, PLOTLY_FONT } from "./theme";
...
export function ManhattanPlot({
  ...
  const { traces, tickVals, tickText, maxX } = useMemo(() => {
    ...
    for (const c of chroms) {
      ...
      chromRanges[c] = [
        Math.min(...pts.map((p) => p.position)),
        Math.max(...pts.map((p) => p.position)),
      ];
    }
    ...
    for (const pt of data) {
      ...
      const key = hasSig
        ? `${pt.group}|${pt.color}|${pt.symbol}|${isSig}`
        : `${pt.group}|${pt.color}|${pt.symbol}`;
      if (!groups.has(key))
        groups.set(key, {
          x: [],
          y: [],
      ...
      g.y.push(pt.value);
      g.text.push(
        pt.hoverText ?? `chr${pt.chrom}:${pt.position.toLocaleString()}`,
      );
    }

    // Sort: non-significant first (behind), significant on top
    const sorted = [...groups.values()].sort(
      (a, b) => (a.sig ? 1 : 0) - (b.sig ? 1 : 0),
    );
