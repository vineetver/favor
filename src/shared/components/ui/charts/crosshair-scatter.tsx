...
import { PLOTLY_CONFIG_STATIC, PLOTLY_FONT, PLOTLY_AXIS, METHOD_PALETTE, METHOD_SYMBOLS } from "./theme";
...
export function CrosshairScatter({
  data, title, xLabel, yLabel, xRefLine, className,
}: CrosshairScatterProps) {
  const traces = data.map((d) => ({
    type: "scatter" as const,
    x: [d.x], y: [d.y], name: d.label, mode: "markers" as const,
    marker: {
    ...
    error_x: {
      ...
      array: [d.xHi - d.x], arrayminus: [d.x - d.xLo],
      color: METHOD_PALETTE[d.label] ?? "#737373", thickness: 1.5, width: 4,
    },
    error_y: {
      type: "data" as const, symmetric: false,
      array: [d.yHi - d.y], arrayminus: [d.y - d.yLo],
      color: METHOD_PALETTE[d.label] ?? "#737373", thickness: 1.5, width: 4,
    },
  ...
  const shapes = xRefLine != null ? [{
    type: "line" as const, x0: xRefLine, x1: xRefLine,
    ...
    <div className={className}>
      <Plot
        ...
        layout={{
          ...
          title: title ? { text: `<b>${title}</b>`, font: { size: 13 } } : undefined,
          height: 400,
          margin: { l: 60, r: 120, t: title ? 50 : 20, b: 55 },
          showlegend: true,
          legend: { x: 1.02, y: 1, xanchor: "left" as const, font: { size: 11 } },
          xaxis: { ...PLOTLY_AXIS, title: { text: xLabel, font: { size: 11 } } },
          yaxis: { ...PLOTLY_AXIS, title: { text: yLabel, font: { size: 11 } } },
          shapes,
        }}
