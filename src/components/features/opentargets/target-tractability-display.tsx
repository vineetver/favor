...
export function TargetTractabilityDisplay({
  target,
  geneName,
}: TargetTractabilityDisplayProps) {
  const tractability = target.tractability || [];

  const groupedByModality = MODALITY_ORDER.reduce(
    (acc, modality) => {
      acc[modality] = MODALITY_CATEGORIES[modality].map((categoryLabel) => {
        const foundItem = tractability.find(
    ...
    {} as Record<
      string,
      Array<{ label: string; modality: string; value: boolean }>
    >,
  );
