...
export function TissueSpecificSummary({
  variant,
  data,
}: TissueSpecificSummaryProps) {
  const summaries = useMemo(() => {
    // cCREs - Show regulatory elements with types
    const activeCCREs = data.ccre.filter(
      (item) =>
    ...
    const ccreSummary =
      activeCCREs.length > 0
        ? `${activeCCREs.length} regulatory elements (${ccreTypes.join(", ")})`
        : null;

    // ABC Peaks - Show significant chromatin accessibility
    const significantPeaks = data.abcPeaks.filter(
      (item) => item && item.q_value != null && item.q_value < 0.05,
    );
    const peaksSummary =
      significantPeaks.length > 0
        ? `${significantPeaks.length} significant accessibility peaks`
        : null;

    // ABC Scores - Show strong enhancer-gene links with target genes
    const strongLinks = data.abcScores.filter(
      (item) => item && item.abc_score != null && item.abc_score > 0.02,
    );
    const targetGenesSet = new Set(
      strongLinks.map((item) => item.gene_name).filter(Boolean),
    );
    ...
    const tissuesWithExpressionSet = new Set(
      data.entex.map((item) => item?.tissue).filter(Boolean),
    );
    const tissuesWithExpression = Array.from(tissuesWithExpressionSet);
    const entexSummary =
      tissuesWithExpression.length > 0
    ...
    const tissuesWithRegulationSet = new Set(
      data.scent.map((item) => item?.tissue).filter(Boolean),
    );
    const tissuesWithRegulation = Array.from(tissuesWithRegulationSet);
    const scentSummary =
      tissuesWithRegulation.length > 0
    ...
    const strongGeneLinks = data.pgboost.filter(
      (item) =>
        item &&
        item.pg_boost_percentile != null &&
        item.pg_boost_percentile > 0.75,
    );
    const topGenes = strongGeneLinks
      .slice(0, 3)
      .map((item) => item?.gene)
      .filter(Boolean);
    const pgboostSummary =
      topGenes.length > 0
  ...
  const summaryText = useMemo(() => {
    switch (overallSignificance) {
      case "high":
        return "Strong tissue-specific regulatory activity";
      case "medium":
        return "Moderate tissue-specific activity";
      case "low":
        return "Limited tissue-specific activity";
      default:
        return "No significant tissue-specific activity";
    }
  }, [overallSignificance]);
      ...
      <CardContent className="space-y-4">
        ...
        {summaries.length > 0 ? (
          ...
          <div className="text-sm text-muted-foreground">
            No tissue-specific regulatory activity detected in available data
            sources.
          </div>
        )}
      </CardContent>
