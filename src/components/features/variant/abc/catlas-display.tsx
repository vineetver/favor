...
import {
  abcPeaksColumns,
  abcScoreColumns,
} from "@/lib/variant/abc/table-columns";
...
export function CatlasDisplay({ peaks, scores }: CatlasDisplayProps) {
  const peaksChartData = useMemo(() => {
    ...
    if (processed.length > 1000) {
      return processed.sort((a, b) => b.neg_log_p - a.neg_log_p).slice(0, 1000);
    }
    return processed;
  }, [peaks]);
  ...
  const tissueOptions = useMemo(() => {
    const tissueSet = new Set([
      ...peaks.map((p) => p.tissue),
      ...scores.map((s) => s.tissue),
    ]);
        ...
        peaksChartData.length > 0 ? (
          <ScatterChart
            ...
            formatTooltipValue={(value, name) => {
              ...
              if (name === "signal_value" || name === "abc_score")
        ...
        scores.length > 0 ? (
          <DataGrid
            ...
            emptyState={{
              ...
              description:
                "No ABC enhancer-promoter links match your current filters.",
              dataType: "links",
            }}
          />
        ) : (
