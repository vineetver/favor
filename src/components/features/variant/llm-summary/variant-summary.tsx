...
export function VariantSummary({ vcf, modelId = "gpt-4o-mini" }: VariantSummaryProps) {
  ...
  const checkCache = async (): Promise<CacheResponse | null> => {
    try {
      const response = await fetch(`/api/variant-summary-cache?vcf=${encodeURIComponent(vcf)}`);
  ...
  const triggerGeneration = async () => {
    try {
      const response = await fetch("/api/variant-summary", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vcf, model: modelId }),
      });
  ...
  useEffect(() => {
    const initializeSummary = async () => {
      if (!vcf || hasTriggeredGeneration.current) return;
