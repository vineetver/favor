...
import { ChevronDown, Code2, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
...
>(({ type, state, className, ...props }, ref) => {
  ...
  const displayName = type.replace('tool-', '').replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
