...
import { processBatchAnnotation, formatBatchResults } from '@/lib/hg19/batch/processor';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const coordinateSystem = formData.get('coordinate-system') as '1-base' | '0-base';
    ...
    // Format results
    const format = outputFormat?.includes('json') ? 'json' 
                 : outputFormat?.includes('tsv') ? 'tsv' 
                 : 'csv';
