...
import type {
  CategoryDef,
  CorrelationData,
} from "../data/_lib/load-correlation-data";
...
function corrColor(v: number): string {
  ...
  if (v >= 0)
    return `rgb(255,${255 - Math.round(t * 168)},${255 - Math.round(t * 213)})`;
...
function cellVal(a: Item, b: Item, d: CorrelationData): number {
  ...
  const ar =
    a.kind === "block" ? rangeFor(categories[a.category]) : [a.annotationIdx];
  const br =
    b.kind === "block" ? rangeFor(categories[b.category]) : [b.annotationIdx];
      ...
      {items.map((it, i) => {
        ...
        return (
          <text
            ...
            textAnchor="end"
            dominantBaseline="central"
            fontSize={block ? FONT_CAT : FONT_ANN}
...
function InteractiveMatrix({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((e) =>
      setAvailW(e[0]?.contentRect.width ?? 900),
    );
    ro.observe(el);
    return () => ro.disconnect();
  }, []);

  const { cell, matrixPx, labelW, colHeaderH } = computeLayout(
    items,
    availW,
    56,
  );
  ...
  const clear = useCallback(() => {
    prev.current = "";
    if (chRow.current) {
      chRow.current.style.display = "none";
    }
    if (chCol.current) {
      chCol.current.style.display = "none";
    }
    if (tipRef.current) {
    ...
    // Reset column labels
    wrapRef.current
      ?.querySelectorAll<SVGTextElement>("[data-cl]")
      .forEach((el) => {
  ...
  const onMove = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      if (tip && wrap) {
        ...
        const aL =
          ci[row].kind === "annotation" ? ci[row].displayLabel : ci[row].label;
        const bL =
          ci[col].kind === "annotation" ? ci[col].displayLabel : ci[col].label;
    ...
    <div ref={wrapRef} className="relative w-full">
      ...
      <div className="flex items-start">
        ...
        <div
          ...
          style={{ width: labelW, paddingTop: colHeaderH }}
        >
          {items.map((it, i) => {
            const block = it.kind === "block";
            const color = data.categories[it.category].color;
            return (
              <div
                key={i}
                data-rl={i}
                className="flex items-center justify-end pr-2 origin-right"
                style={{
                  height: cell,
                  transition:
                    "opacity 0.15s, transform 0.15s, font-weight 0.1s",
                }}
              >
                {block ? (
                  <button
                    type="button"
                    onClick={() => onToggle(it.category)}
        ...
        <div className="min-w-0 overflow-x-auto" style={{ flex: "1 1 0%" }}>
          <svg
            width={matrixPx}
            height={colHeaderH + matrixPx}
            className="block select-none"
            style={{ minWidth: matrixPx, cursor: "crosshair" }}
            onMouseMove={onMove}
            onMouseLeave={clear}
            onClick={onClick}
          >
        ...
        <div
          className="shrink-0 pl-4 select-none"
          style={{ paddingTop: colHeaderH, width: legendColW }}
        >
          <svg
            width={LEGEND_W}
            height={Math.min(260, matrixPx) + LEGEND_TITLE_H + 8}
