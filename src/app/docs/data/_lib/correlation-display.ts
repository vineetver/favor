...
const PREFIX_RULES: [RegExp, Replacer][] = [
  ...
  [
    /^dbnsfp_(.+?)(?:_score)?$/,
    (m) => m[1].split("_").map(capitalize).join(" "),
  ],
  // gnomAD exome/genome AFs
  [
    /^gnomad_(exome|genome)_(?:populations\.)?(\w+?)_af(?:_(xx|xy))?$/,
    (m) => {
  ...
  [
    /^gnomad_(exome|genome)_af(?:_(xx|xy))?$/,
    (m) => {
      const src = m[1] === "exome" ? "ex" : "ge";
  ...
  [
    /^gnomad_(exome|genome)_faf_(.+)$/,
    (m) => `gnomAD ${m[1] === "exome" ? "ex" : "ge"} ${m[2].toUpperCase()}`,
  ],
  // gnomAD functional scores
  [
    /^gnomad_(exome|genome)_functional_(.+)$/,
    (m) =>
      `gnomAD ${m[1] === "exome" ? "ex" : "ge"} ${m[2].split("_").map(capitalize).join(" ")}`,
  ],
  // Conservation
  ...
  [
    /^apc_(.+?)(?:_v\d+)?$/,
    (m) => `aPC-${m[1].split("_").map(capitalize).join("-")}`,
  ],
  ...
  [
    /^spliceai_(.+)$/,
    (m) =>
      `SpliceAI ${m[1].replace("ds_", "").replace("max_ds", "max").toUpperCase()}`,
  ],
  // CV2F / MACIE
