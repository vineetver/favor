...
export default async function PerturbationPage({ params }: PerturbationPageProps) {
  ...
  const [downstreamRes, upstreamRes, crisprRes, maveRes] = await Promise.all([
    fetchPerturbSeq(geneSymbol, { significant_only: true, limit: 100 }).catch(() => null),
    fetchPerturbSeq(geneSymbol, { effect_gene: geneSymbol, significant_only: true, limit: 50 }).catch(() => null),
