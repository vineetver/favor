...
import { useCosmicData } from "@/lib/gene/cosmic/hooks";
import { cosmicColumns, type Cosmic } from "@/lib/gene/cosmic/columns";

export default function GeneCosmicPage() {
  const { geneName } = useParams();
  const { data, isLoading, error } = useCosmicData(geneName as string);
  ...
  const handleExport = (filteredData: Cosmic[]) => {
    ...
    a.href = url;
    a.download = `cosmic-gene-${geneName}-${new Date().toISOString().split('T')[0]}.tsv`;
    a.click();
  ...
  return (
    <div className="container mx-auto py-8">
      <DataGrid
        ...
        data={data || []}
        isLoading={isLoading}
        title={`COSMIC Cancer Mutations - ${geneName}`}
        description="Cancer somatic mutations from the COSMIC database for this gene"
        searchPlaceholder="Search mutations..."
        facetedFilters={facetedFilters}
        onExport={handleExport}
        exportFilename={`cosmic-gene-${geneName}-${new Date().toISOString().split('T')[0]}.tsv`}
        initialPageSize={25}
        emptyState={{
          title: "No COSMIC data found",
          description: "No cancer mutations found for this gene.",
          dataType: "mutations"
        }}
      />
    </div>
