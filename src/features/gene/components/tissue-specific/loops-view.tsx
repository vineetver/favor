      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {rows.length} loop{rows.length !== 1 ? "s" : ""} &middot;
            arc height proportional to span &middot; color = tissue
          </p>
        </div>
        ...
        <div className="px-4 py-3">
          ...
          >
            ...
            <line
              x1={0} y1={TRACK_Y}
              x2={SVG_WIDTH} y2={TRACK_Y}
              stroke="currentColor"
              strokeWidth={1}
              className="text-border"
            />
            <text x={2} y={TRACK_Y + 14} className="text-muted-foreground" fill="currentColor" fontSize={9} fontFamily="monospace">
              {Math.round(minCoord).toLocaleString()}
            </text>
            <text x={SVG_WIDTH / 2} y={TRACK_Y + 14} textAnchor="middle" className="text-muted-foreground" fill="currentColor" fontSize={9} fontFamily="monospace">
              {midCoord.toLocaleString()}
            </text>
            <text x={SVG_WIDTH - 2} y={TRACK_Y + 14} textAnchor="end" className="text-muted-foreground" fill="currentColor" fontSize={9} fontFamily="monospace">
            ...
            {sortedArcs.map((loop) => {
              ...
              const arcHeight = ARC_AREA_TOP + arcSpanPct * (TRACK_Y - ARC_AREA_TOP - 20);
                  ...
                  <TooltipContent side="top" className="text-sm max-w-xs">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-muted-foreground text-xs">
                        Anchor 1: {loop.anchor1_start.toLocaleString()}&ndash;{loop.anchor1_end.toLocaleString()}
                      </p>
                      <p className="text-muted-foreground text-xs">
                        Anchor 2: {loop.anchor2_start.toLocaleString()}&ndash;{loop.anchor2_end.toLocaleString()}
                      </p>
                      <p className="text-muted-foreground text-xs">
                        Span: {loop.loop_span.toLocaleString()} bp &middot; {loop.assay_type}
                      </p>
                    </div>
                  </TooltipContent>
                </Tooltip>
            ...
            {rows.map((loop, i) => {
              ...
              return (
                <g key={`anchors-${i}`}>
                  <rect
                    x={a1x1} y={TRACK_Y - 3}
                    width={Math.max(a1x2 - a1x1, 1.5)} height={6}
                    rx={0.5}
                  ...
                  <rect
                    x={a2x1} y={TRACK_Y - 3}
                    width={Math.max(a2x2 - a2x1, 1.5)} height={6}
                    rx={0.5}
                    fill={tissueColor(loop.tissue_name, tissues)}
                    opacity={0.5}
                  />
