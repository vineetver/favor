...
export const geneTractabilityTargetClassColumns = [
  col.accessor("tractability", {
    ...
    cell: cell.custom<Gene, any>((tractability: Array<{ modality: string; id: string; value: boolean }>) => {
      if (!tractability || tractability.length === 0) return null;

      const grouped = tractability.reduce((acc, item) => {
        if (!acc[item.modality]) {
          acc[item.modality] = [];
        }
        acc[item.modality].push(item);
        return acc;
      }, {} as Record<string, Array<{ modality: string; id: string; value: boolean }>>);
  ...
  col.accessor("target_class", {
    ...
    cell: cell.custom<Gene, any>((targetClass: Array<{ id: number; label: string; level: string }>) => {
      if (!targetClass || targetClass.length === 0) return null;

      const grouped = targetClass.reduce((acc, item) => {
        if (!acc[item.level]) {
          acc[item.level] = [];
        }
        acc[item.level].push(item);
        return acc;
      }, {} as Record<string, Array<{ id: number; label: string; level: string }>>);
