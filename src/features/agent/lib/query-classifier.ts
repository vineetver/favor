...
export type QueryRoute = { type: "explanation_only" } | { type: "full_agent" };
...
const EXPLANATION_PATTERNS =
  /^(what does|what is|what are|meaning of|explain|in simpler terms|summarize|what tools|how many|which of those|tell me more about (that|the|those)|can you clarify|what do you mean|rephrase|say that again|break that down|why is that|how does that|what happened)\b/i;

const NEW_ENTITY_PATTERNS =
  /\b(look up|search for|find|analyze|run enrichment|get .+ for|compare|what about)\b/i;

const ENTITY_NAME_PATTERNS =
  /\b(rs\d+|ENSG\d+|chr\d+[:-]\d+|[A-Z][A-Z0-9]{1,10}[0-9])\b/;
...
const ACKNOWLEDGEMENT_PATTERN =
  /^(ok|okay|thanks|thank you|got it|sure|yes|no|yep|nope|cool|great|nice|perfect|understood|i see|makes sense)\.?$/i;
