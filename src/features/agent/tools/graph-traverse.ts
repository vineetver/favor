...
export const graphTraverse = tool({
  description:
    "Advanced: Execute a multi-step graph traversal query. This is the most powerful tool — use simpler tools first (searchEntities, getRankedNeighbors, etc.) and only use this for complex multi-hop queries that can't be answered otherwise.",
  ...
  execute: async ({ seeds, steps, nodeFields, edgeFields }) => {
    ...
    // Cap output for LLM consumption
