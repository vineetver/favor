 ...
 * Persists parquet data across page refreshes to avoid re-downloading
 */

const DB_NAME = "favor-parquet-cache";
const DB_VERSION = 1;
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (age > CACHE_TTL_MS) {
          ...
          deleteCachedParquet(url).catch(console.error);
          resolve(null);
          return;
        }
 ...
 */
export async function deleteCachedParquet(url: string): Promise<void> {
  try {
    const db = await openDatabase();
 ...
 */
export async function clearParquetCache(): Promise<void> {
  try {
    const db = await openDatabase();
...
/**
 * Check if parquet file is cached
 */
export async function isParquetCached(url: string): Promise<boolean> {
