...
export function parseRegion(input: string): ParseResult<GenomicRegion> {
  ...
  if (!region) {
    return {
      ok: false,
      error: 'Invalid region: start must be less than end and both must be positive'
    }
  }
 ...
 */
export function formatRegion(region: GenomicRegion, options?: { commas?: boolean }): string {
 ...
 */
export function panLeft(region: GenomicRegion, fraction: number = 0.25): GenomicRegion | null {
 ...
 */
export function panRight(region: GenomicRegion, fraction: number = 0.25): GenomicRegion | null {
  const shift = Math.floor(region.size * fraction)
 ...
 */
export function centerOn(region: GenomicRegion, position: number): GenomicRegion | null {
