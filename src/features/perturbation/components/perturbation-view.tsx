...
import type { CrisprRow, MaveRow, PerturbSeqRow } from "../types";
...
function UpstreamSection({
  ...
  const columns = useMemo(() => {
    const cols = buildPerturbSeqColumns(
      {
...
const maveColumns: ColumnDef<MaveRow, unknown>[] = [
  ...
  },
  {
    id: "aa_change",
    accessorFn: (r) =>
      r.aa_wt && r.aa_change ? `${r.aa_wt}→${r.aa_change}` : null,
    header: "AA Change",
    enableSorting: false,
    cell: ({ row }) => {
      ...
      return (
        <span
          className={cn(
            "text-xs",
            isSyn ? "text-muted-foreground" : "text-foreground",
          )}
        >
          {aa_wt}
          {isSyn ? " (syn)" : `→${aa_change}`}
...
function MaveSection({
  ...
  const datasets = useMemo(
    () => new Set(data.map((r) => r.dataset_id)).size,
    [data],
  );
  return (
    <DataSurface
      ...
      exportable
      exportFilename={`mave-scores-${geneSymbol}`}
      emptyMessage={`No MAVE data found for ${geneSymbol}`}
    />
  );
}
